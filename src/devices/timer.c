...
timer_sleep (int64_t ticks) 
{
  ...
  //calculate ticks to put thread to sleep for
  thread_current()->sleep_for_ticks = timer_ticks() + ticks;
  
  //add threaad to list of sleeping list (it is ordered)
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  while(sleep_list_elem != list_end(&sleeping_list))
  {
    struct thread *thread_to_unblock = list_entry(sleep_list_elem, 
                                      struct thread, elem);
    if(ticks < thread_to_unblock->sleep_for_ticks){
      return;
    }
    //removing from list of sleeping threads
